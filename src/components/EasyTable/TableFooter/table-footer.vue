<template>
  <table cellspacing="0" cellpadding="0" border="0" :style="{ width: bodyWidth }">
    <colgroup>
    ...
    <tbody :class="{ 'has-gutter': hasGutter }">
      <tr v-for="(row, rowIndex) in data" :key="rowIndex">
        <td v-for="(column, cellIndex) in columns" :key="column.id" :class="getRowClasses(column, cellIndex)">
          <render-cell :row="row" :column="column" :cell-index="cellIndex" :cell="$parent.$scopedSlots[column.property]" />
        </td>
        <th v-if="hasGutter" class="gutter" />
      </tr>
    </tbody>
  </table>
</template>
...
export default {
  ...
  destroyed() {
    if (this.tableLayout && this.tableLayout.removeObserver) { this.tableLayout.removeObserver(this) }
  },
  methods: {
    init() {
